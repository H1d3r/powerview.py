...
COMMANDS = {
    ...
    'Get-DomainDNSRecord':['-ZoneName','-Identity','-Legacy','-Forest','-RecordType','-Properties','-SearchBase','-Server','-Select','-Where', '-Count', '-NoWrap', '-TableView', '-SortBy', '-OutFile', '-NoCache', '-NoVulnCheck', '-Raw'],
    ...
    'Invoke-DMSASync':['-DMSAName', '-PrincipalAllowed', '-TargetIdentity', '-Force', '-BaseDN', '-Server','-NoCache', '-TableView', '-OutFile', '-Count', '-NoWrap', '-NoDelete'],
    ...
    'Enable-RDP':['-Computer', '-NoVerify', '-DisableRestrictedAdmin'],
    'Disable-RDP':['-Computer', '-NoVerify'],
    'Enable-ShadowRDP':['-Computer'],
    'Disable-ShadowRDP':['-Computer'],
    ...
    'Remove-DomainObject':['-Identity','-BaseDN','-Legacy','-Forest','-NoCache','-Server','-OutFile'],
    ...
    'Get-DomainObjectAcl':['-Identity','-LDAPFilter','-SearchBase','-Server','-SecurityIdentifier','-ResolveGUIDs','-Depth','-Select', '-Where', '-Count', '-NoWrap', '-TableView', '-OutFile', '-NoCache', '-NoVulnCheck', '-Raw'],
    'Get-ObjectAcl':['-Identity','-LDAPFilter','-SearchBase','-Server','-ResolveGUIDs','-SecurityIdentifier','-Depth','-Select', '-Where', '-Count', '-NoWrap', '-TableView', '-OutFile', '-NoCache', '-NoVulnCheck', '-Raw'],
    'Get-DomainComputer':['-Identity','-Properties','-ResolveSIDs','-LDAPFilter','-SearchBase','-Server','-Select','-Enabled','-Disabled','-Workstation','-NotWorkstation','-Obsolete','-Unconstrained','-TrustedToAuth', '-LAPS', '-BitLocker', '-RBCD', '-ShadowCred','-SPN','-GMSAPassword','-Pre2K','-Printers','-ExcludeDCs','-IncludeIP','-Where', '-Count', '-NoWrap', '-TableView', '-SortBy', '-OutFile', '-NoCache', '-NoVulnCheck', '-Raw'],
    ...
    'Add-DomainDNSRecord':['-ZoneName','-RecordName','-RecordAddress','-BaseDN','-Legacy','-Forest','-NoCache','-Server', '-OutFile'],
    ...
    'Remove-GPLink':['-GUID','-TargetIdentity','-TargetSearchBase','-GPOSearchBase','-Server','-OutFile'],
